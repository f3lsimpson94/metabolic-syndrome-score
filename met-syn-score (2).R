...
# cognition LM STM PS Attention

# 2)Dietary Assessment (3 diets: planty, meat, healthy)
...
PS5 = update(PS4, .~. + meat.diet:MetSSS)
anova(PS5,PS4)
...
#  want a D-W statistic close to 2 and a non-significant p value
...
STM = lm(data = gaucci,ShortTermMem  ~ visit_age+sex+educationtotal)
summary(STM)

STM2 = lm(data = gaucci, ShortTermMem ~ visit_age+sex+ meat.diet + plant.diet + western.diet)
summary(STM2)
